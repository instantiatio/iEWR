...
process.stdout.write(`<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; style-src 'unsafe-inline'; base-uri 'none'; form-action 'none'"><title>${esc(title)}</title><style>${input.css}\nmain{max-width:1000px}h2{margin:24px 0 10px;font-size:20px}h3{margin-top:20px}table{display:block;overflow-x:auto;margin:15px 0}p,li{overflow-wrap:anywhere}.source-note{padding-bottom:12px;margin-bottom:18px;border-bottom:1px solid var(--border)}pre{padding:12px;background:var(--surface)}a small{font-size:.85rem}.download summary{color:var(--accent);cursor:pointer}</style></head><body><main><aside class="source-note"><p>Оформленное представление материала · ${esc(input.revision||'')}</p><p class="hint">Источник: <a href="${esc(sourceLink)}" target="_blank" rel="noopener noreferrer">${esc(path.basename(input.sourcePath))} (MD, текст)</a>. Ссылки «исходник MD» ведут к исходным файлам.</p><p class="hint">Если исходник недоступен, запросите у Developing Engineer файл „${esc(path.basename(input.sourcePath))}“ — ${esc(title)}. SHA-256: ${esc(input.sourceSHA256)}.</p></aside><article>${body}</article></main></body></html>`);
