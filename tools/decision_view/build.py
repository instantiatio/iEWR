...
def render(source, output, source_dir):
    ...
    for material in d.get('materials', []):
        ...
        if not parsed.scheme:
            ...
            if local.suffix.lower() == '.md':
                ...
                if reader.exists():
                    if reader.read_text()!=rendered:raise ValueError('Existing reader differs; preserve it and issue a new representation')
            ...
            if local.name.endswith('.er.json'):
                ...
                if reader.exists():
                    if reader.read_text()!=rendered:raise ValueError('ER reader collision')
                else:write_new(reader,rendered)
                material['source']={'path':quote(os.path.relpath(local,output.parent),safe='/'),
                                    'filename':local.name,'sha256':digest}
    ...
    if d.get('details'):
        blocks.append('<details><summary>Точный предмет и основание</summary><p>'+esc(d['details'])+'</p></details>')
    ...
    replacements={'TITLE':d.get('title',d['question']), 'CSS':(HERE/'style.css').read_text(), 'CONTENT':''.join(blocks), 'PARAMETERS':parameter_html, 'DATA':json_text(d), 'APP':(HERE/'app.js').read_text()}
    replacements['TITLE']=esc(replacements['TITLE'])
    template=(HERE/'page.html.in').read_text()
...
if __name__ == '__main__':
    ...
    print(json.dumps(build(json.loads(a.data.read_text()),a.output,a.data.parent),ensure_ascii=False,indent=2))
