...
def render(model,source_name='',source_sha=''):
    ...
    for index,entity in enumerate(entities):
        ...
        node=f'<g class="entity" id="entity-{esc(entity["id"])}"><rect x="{x}" y="{row_y}" width="{box_w}" height="{height}" rx="7" fill="white" stroke="#547261"/>'
        node+=f'<path d="M{x},{row_y+header} H{x+box_w}" stroke="#9eb6a6"/>'
...
body{margin:0;background:#f4f5f1;color:#24382d;font:14px/1.5 system-ui,sans-serif}main{max-width:1200px;margin:20px auto;padding:24px;background:white;border:1px solid #dde4db;border-radius:8px}h1{font-size:25px;margin:6px 0}h2{font-size:18px;margin-top:24px}.meta{color:#647467;font-size:12px;overflow-wrap:anywhere}.controls[hidden]{display:none}.controls{display:flex;flex-wrap:wrap;gap:9px;align-items:center;margin:15px 0}button{font:inherit;padding:6px 12px;border:1px solid #aac1af;border-radius:4px;background:#f4f8f1;cursor:pointer}button:focus-visible{outline:3px solid #668f74}.viewport{overflow:auto;border:1px solid #d6e1d4;border-radius:6px;max-height:75vh;cursor:grab;touch-action:pan-x pan-y}.viewport.dragging{cursor:grabbing}svg{display:block;max-width:none}svg text{paint-order:stroke;stroke:white;stroke-width:3px;stroke-linejoin:round}.table{overflow:auto}table{border-collapse:collapse;width:100%}th,td{padding:9px;text-align:left;border-bottom:1px solid #dce4d8;vertical-align:top}pre{white-space:pre-wrap}code{overflow-wrap:anywhere}@media(max-width:640px){main{margin:0;padding:18px;border:0}}@media print{.controls{display:none}.viewport{overflow:visible;max-height:none}svg{width:100%!important;height:auto!important}}
