# Переход с 5.2.2 через линию 5.3 к iEWR 5.5.3 Beta

## С 5.5.2 к 5.5.3

Обновлены SYSE и EXD, добавлен DOCA; остальные восемь авторских DPF и локальный
SDLC сохранены. У SYSE и EXD новые идентификаторы редакций: старые привязки
источников и основания решений не переписывать. Проверять только затронутое
применение. Форматы, программные интерфейсы и контракт ядра не изменены.

При переносе сохранять исходный архив и материалы проекта отдельно от файлов
нового продукта. Приветствие закреплено в AGENTS.md; заметка о текущем выпуске
находится в RELEASE_NOTES.md, накопленная история — в RELEASE_HISTORY.md.
Подключение DOCA не требует загрузки всех его внешних зависимостей заранее.
