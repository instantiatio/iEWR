...
Статус: **опубликованный Beta-выпуск**. [iEWR 5.5.5 Beta](https://github.com/instantiatio/iEWR/releases/tag/5.5.5-beta) — текущий Latest.
[Скачать ZIP 5.5.5](https://github.com/instantiatio/iEWR/releases/download/5.5.5-beta/iEWR-5.5.5-Beta.zip); результаты и ограничения — во вложении [VERIFICATION.md](https://github.com/instantiatio/iEWR/releases/download/5.5.5-beta/VERIFICATION.md).
Интеграция с DSH удалена. Сохранены переносимые правила восстановления и
необязательный помощник проверки. Обычное использование основано на инструкциях
проекта и доступных инструментах выбранной среды.
Это основной README поставки: назначение, начало работы, использование и границы.

- [Изменения 5.5.5](RELEASE_NOTES.md) — что изменилось, условия перехода и ограничения. [История 5.2.2–5.5.4](RELEASE_HISTORY.md) сохранена отдельно.
- [Состав поставки](PACKAGE_MANIFEST.md) и [исходная основа и статус](BASELINE_STATUS.md) — точные файлы и границы проверок.

## Содержание

- [Что такое iEWR](#что-такое-iewr)
- [Зачем он нужен](#зачем-он-нужен)
- [Начало работы](#начало-работы)
- [Взаимодействие и подача результата](#взаимодействие-и-подача-результата)
- [Подключение внешнего DPF](#подключение-внешнего-dpf)
- [Размещение материалов проекта](#размещение-материалов-проекта)
- [Саморазвитие iEWR](#саморазвитие-iewr)
- [Как работает iEWR](#как-работает-iewr)
- [Архитектура](#архитектура)
- [FPF, DPF и локальная практика](#fpf-dpf-и-локальная-практика)
- [Инженерные принципы](#инженерные-принципы)
- [Границы назначения](#границы-назначения)
- [Происхождение: iDPF → iEWR](#происхождение-idpf--iewr)
- [Статус, выпуски и документация](#статус-выпуски-и-документация)
- [Проверка пакета и исходной конфигурации DPF](#проверка-пакета-и-исходной-конфигурации-dpf)
- [Лицензии и авторские источники](#лицензии-и-авторские-источники)
...
Инструкции проекта находятся в [AGENTS.md](AGENTS.md). При первом приветствии ассистент представляется как ИИ-ассистент инженерной среды iEWR. Он предлагает описать задачу или добавить материалы в `project/source/` и помочь с Formation — подготовкой необходимой инженерной деятельности под результат. Для простого вопроса достаточно прямого ответа или действия.

Для начала работы не нужно знать FPF, названия модулей или выбирать режим процесса. iEWR определяет, новая это задача или продолжение, и уточняет только сведения, необходимые для результата или разрешения действия.
...
[Единый указатель](catalog/dpf/METHODS.md) помогает перейти от вопроса к DPF,
методу и точному разделу оригинала. Агент читает необходимые действия, условия
и ограничения, при необходимости — примеры и связанные разделы. Весь DPF читать
не требуется. Достаточная прочитанная основа используется повторно; карточки
и копии разделов при чтении не создаются.
...
**LPF — описание локальной практики — необязателен.** Если у проекта есть оформленная локальная практика, укажите её источник и область применения. iEWR работает и без неё.
...
### Codex и другие агентные среды

Распакуйте ZIP в новый каталог и откройте внутренний каталог `iEWR-5.5.5-Beta`,
содержащий `AGENTS.md`, как рабочий проект. Среда должна поддерживать загрузку
инструкций проекта. Для загрузки инструкций и начала работы не требуются
специальный plugin, Python, Node.js или программный Recovery entry. Отдельные
сборщики и проверки имеют собственные требования к запуску. Исходные
материалы размещайте в `project/source/`, результаты — в `project/artifacts/`.
Техническое исполнение ограничено фактическими разрешениями и controls среды.

### Восстановление после прерывания

Перед продолжением агент восстанавливает действующие разрешения и ограничения,
использованные редакции материалов и фактические действия/последствия. Если
выбор инициативы уже прямо установлен, он используется повторно. Если оснований
недостаточно, агент указывает конкретный пробел; неизвестный эффект не повторяет.

Для этого достаточно пригодной истории и исходных записей: обязательной формы
JSON нет. При необходимости подготовленные сведения можно проверить
[read-only helper](app/bootstrap/ENTRY.md), для которого нужен Python 3.10+.
Он не добывает недостающие разрешения и не создаёт записи вместо их владельцев.
Его результат не даёт права на действие. Техническую защиту инструментов и
сохранность истории предоставляет выбранная среда; инструкции их не гарантируют.

## Взаимодействие и подача результата

Когда нужен человеческий выбор, [Decision View — представление для решения](modules/interaction/DECISION_VIEW.md)
показывает предмет, основания, существенные альтернативы, неопределённость и
действие после ответа. При предъявлении готового результата специалисту на
утверждение, согласование или приёмку агент сразу готовит переносимый Decision View
вместе с техническими материалами. Отдельного напоминания не требуется; это не
создаёт нового согласования. Для рабочих уточнений достаточно разговора; явный
выбор человеком другой формы сохраняется. Уже разрешённое продолжается;
ожидание удерживает только зависимое действие. Ответ применяется с его условиями,
а выполненное действие сверяется с результатом. Фоновое ожидание и уведомления
доступны лишь если это поддерживает среда исполнения.

Для отдельного переносимого вопроса в поставке есть [каркас Decision View](tools/decision_view/README.md): одна HTML-страница, один вопрос и один возвращаемый ответ. Связанные Markdown и явно заданные ER-модели получают отдельные представления по ссылкам. Готовые файлы работают без сервера; вложения в вопрос не добавляются.

Когда ситуацию, изменение, план, предмет проверки или результат удобнее показать
отдельным документом, доступны [Human Views — представления для человека](tools/human_view/README.md): пять
композиций для чтения в общем визуальном языке. По умолчанию достаточно разговора;
документ создаётся для конкретной задачи читателя. Режимы «обзор» (GLANCE), «обычное чтение» (STANDARD) и
«подробности» (DEEP) меняют порядок чтения и раскрытие деталей, сохраняя существенные условия и выводы.
В шапке видны тип и режим; файловые сведения доступны в отдельном техническом
разделе. Локальные ссылки новых страниц используют разделитель `/` во всех средах.
HTML работает без сети и JavaScript; открытие и сохранение не являются приёмкой.
Для формального ответа сохраняется Decision View. [Правила выбора](modules/interaction/HUMAN_VIEWS.md)
помогают подготовить содержание; сборщик не определяет контекст и не оценивает
значимость автоматически. Улучшение человеческого времени чтения не измерено.

В Decision View человек выбирает вариант либо пишет своими словами, сохраняет HTML-ответ и явно
возвращает его в задачу. Сохранение само ничего не отправляет и не применяет.
Агент связывает точный ответ с вопросом и сохраняет получение в контексте проекта
до зависимого действия. Другой завершённый ответ разбирается вручную.

Для сборки HTML нужны Python 3.10+ и, только при оформлении Markdown, Node.js 20+.
Marked уже включён в комплект. Готовому файлу нужен обычный браузер; Python,
Node.js и сервер для просмотра не требуются. JavaScript нужен для заполнения и
скачивания ответа; сохранённый ответ читается и без него. Учебный пример из корня:

```sh
python3 -B tools/decision_view/build.py tools/decision_view/examples/question.json project/handoff/example-question.html
```

Откройте полученный `project/handoff/example-question.html` как файл. Ссылка на
ER-схему ведёт к отдельному представлению. Подготовка данных, приём ответа и
пределы форматов описаны в [руководстве каркаса](tools/decision_view/README.md).

При нескольких существенных ожиданиях можно запросить «Покажи ожидаемые решения
по этой задаче». Агент отделяет готовые вопросы от недостающих входов/адресатов
и полученных ответов с условиями, объясняет порядок и показывает независимое
продолжение. При необходимости создаётся [статический список](tools/decision_view/README.md#список-ожидаемых-решений)
со ссылками на отдельные вопросы, моментом снимка и непроверенными основаниями.
Список не рассылает и не применяет ответы; перед действием основания сверяются.
Для одного простого вопроса достаточно разговора. Экономия внимания ещё не измерена.
...
Поместите DPF в [external-sources/external-dpf/](external-sources/external-dpf/), желательно сохранив исходное имя файла. Затем попросите:

```text
Зарегистрируй DPF из external-sources/external-dpf/MUSIC-AND-DANCE-PRACTICE-ENGINEERING-PRINCIPLES-FRAMEWORK.md
```

iEWR прочитает источник, установит его обозначение и редакцию, вычислит
контрольную сумму, сохранит необходимые метаданные и внесёт его в реестр DPF
проекта. При постоянном подключении дополнит единый проектный указатель:
предметные вопросы и полный перечень методов со ссылками на исходные разделы.
Достаточное авторское оглавление используется повторно с проверкой охвата.
Отдельные карточки не создаются. При смене редакции проверяются навигация и
условия, затрагивающие текущую работу. Ограничения «только индекс» и запрет
записи соблюдаются; незавершённая часть называется явно. См.
...
В каталоге `project/` размещаются материалы конкретного проекта. Файлы iEWR и
встроенные DPF размещены за его пределами.

| Путь | Назначение | Когда использовать |
|---|---|---|
| `project/source/` | Входящие документы, исходные архивы, данные и возвращённые экземпляры | При получении материалов; оригиналы сохраняются с происхождением |
| `project/handoff/` | Вопросы, замечания, формы выбора и совместные рабочие черновики | Для текущего обсуждения и согласования |
| `project/artifacts/` | Требования, модели, отчёты, код, проверки и комплекты передачи | Для созданных результатов; рабочий файл можно править на месте |

Рабочий документ можно править на месте. Конкретная редакция, на которую опираются
решение, проверка или передача, сохраняется; её исправление создаёт следующую
редакцию. Оригиналы, зафиксированные результаты и необходимые основания
по умолчанию не удаляются автоматически. Достаточность метаданных обосновывается
дальнейшим использованием; пригодная история не дублируется.
Подробности — [правила материалов проекта](project/README.md).
...
По такому явному поручению агент применяет [правила саморазвития iEWR](modules/self_development/GUIDANCE.md): устанавливает точный состав исходной поставки, обосновывает изменение, проверяет его и предъявляет новый архив с результатами проверок и полезным опытом. Обычные пользовательские задачи и регистрация DPF эти правила не включают.
...
**Полный [Engineering DPF Suite](https://github.com/ailev/FPF/tree/main/Engineering%20DPF%20Suite) также не входит в поставку.** Включены одиннадцать избранных авторских предметных рамок:
...
| [Управление операционной деятельностью — OPS](frameworks/dpf/operations-management/) | Продолжение текущей работы, её состояние, обязательства, доступные ресурсы для выполнения работы и улучшение деятельности. |
| [Проектирование объяснений — EXD](frameworks/dpf/explanation-design/) | Объяснение для конкретного получателя, выбор формы и проверка полезности. |
| [Администрирование организации — ADM](frameworks/dpf/organization-administration/) | Разрешения, применимость контроля, пригодный результат и фактическое применение решения. |
| [Исследовательская практика — RMP](frameworks/dpf/research-method-practice/) | Достаточный ответ из источников, исследовательский вопрос и критика знания. |
| [Семантическая интеграция — SIE](frameworks/dpf/semantic-integration-engineering/) | Согласование значений, идентичностей и преобразований разных источников. |
| [Стратегия — STR](frameworks/dpf/strategy/) | Направления развития, горизонты, сценарии и обязательства. |
| [Возможности и направления развития — DOCA](frameworks/dpf/development-opportunity-construction/) | Построение полезных возможностей развития, необходимые условия и совместная достижимость. |
...
SYSE, EXD и DOCA закреплены на редакции Git `4ddaf71557d4159e988cc61d2bd3088bfc1d2803`, остальные восемь авторских DPF — на `453144eda2870a14d47fd9c066deeb0405eee86f`. Авторская дата SYSE — 22 сентября 2026, остальных — 20 сентября. Файлы оригиналов не изменены. Вместе с сохранённым SDLC 0.1.0 база содержит 12 DPF и 197 адресуемых методов.

Часть методов ссылается на источники вне комплекта. Например, DOCA может требовать сведений о развитии человеческих способностей, а инженерное обоснование — специализированных измерений. Эти зависимости проверяются для конкретного применения. Наличие DPF не делает библиотеку полностью автономной и не требует заранее загружать все упомянутые источники.

Расширение базы не требует читать весь корпус. Сначала выбирается источник по
вопросу, затем нужный раздел; достаточная основа переиспользуется. RMP не делает
каждый поиск исследованием, STR — каждый план стратегией, SIE — каждую правку
семантической интеграцией. Влияние расширения на время, токены и качество агента
требует отдельных сравнительных наблюдений; наличие каталога его не доказывает.
...
Состав поставки и ограничения распространения источников указаны в [ведомости файлов](PACKAGE_MANIFEST.md). Изменения текущего выпуска описаны в [краткой заметке](RELEASE_NOTES.md), прежних — в [истории](RELEASE_HISTORY.md).
...
Собственные материалы iEWR распространяются по отдельной [лицензии](LICENSE). Одиннадцать оригинальных DPF Анатолия Левенчука лицензированы по CC BY 4.0. Точные лицензии, сведения об авторстве, редакции и контрольные суммы, отсутствие локальных правок и границы прав на сторонние материалы и программный код сохранены в [уведомлении об источниках](frameworks/dpf/NOTICE.md).
...
В Markdown-сборщике используется включённый Marked 17.0.5 под MIT.
[Сведения о компоненте](tools/decision_view/THIRD_PARTY.md) и
[полный текст лицензии](tools/decision_view/vendor/MARKED_LICENSE.md) входят в поставку.
