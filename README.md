...
- [Изменения 5.5.4](RELEASE_NOTES.md) — что изменилось, условия перехода и ограничения. [История 5.2.2–5.5.3](RELEASE_HISTORY.md) сохранена отдельно.
...
Распакуйте ZIP в новый каталог и откройте внутренний каталог `iEWR-5.5.4-Beta`,
содержащий `AGENTS.md`, как рабочий проект. Среда должна поддерживать загрузку
инструкций проекта. Обычная работа по инструкциям сохраняется; DSH consumer,
Node.js и программный Recovery entry для этого не обязательны. Исходные
...
Распакуйте ZIP в новый workspace; его рабочим корнем должен быть каталог с
`AGENTS.md`, а не внешний каталог архива. Откройте новую session с этим рабочим
каталогом и проверьте загрузку проектных инструкций средствами вашей редакции
DSH. Простая распаковка не активирует программный recovery consumer.

Для его активации требуется отдельное подключение
[по контракту адаптера](adapters/dsh-reentry/README.md): проверенная совместимость
DSH API, Python 3.10+, Node.js 24.18+ и программный bootstrap с доверенными
связями session/records и проверкой конкретного действия. `requireInstalled`
должен выполняться до открытия agent entry. Готовой универсальной настройки
YAML нет; без этих условий native enforcement не заявляется. Установка и live
проверка остаются отдельной работой в явно разрешённом scope.
