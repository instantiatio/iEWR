...
Статус: **опубликованный Beta-выпуск**. [iEWR 5.5.2 Beta](https://github.com/instantiatio/iEWR/releases/tag/5.5.2-beta) — текущий Latest; архив и пределы проверки указаны на странице выпуска.
Это основной README поставки: назначение, начало работы, использование и границы.

- [Release notes: 5.2.2 → 5.5.2](RELEASE_NOTES.md) — накопленные изменения, совместимость, миграция и известные ограничения выпуска.
...
Когда нужен человеческий выбор, [Decision View](modules/interaction/DECISION_VIEW.md)
...
доступны лишь при фактической поддержке host.
...
отдельным документом, доступны [Human Views](tools/human_view/README.md): пять
композиций для чтения в общем визуальном языке. По умолчанию достаточно разговора;
документ создаётся для конкретной задачи читателя. Глубина GLANCE / STANDARD /
DEEP меняет порядок чтения и раскрытие деталей, сохраняя существенные условия и выводы.
В шапке видны тип и режим; файловые сведения доступны в отдельной технической
трассировке. Локальные ссылки новых страниц используют web-разделитель `/` на всех host.
...
помогают подготовить содержание; helper не обнаруживает контекст и не оценивает
значимость автоматически. Улучшение человеческого времени чтения не измерено.

Человек выбирает вариант либо пишет своими словами, сохраняет HTML-ответ и явно
...
**Полный [Engineering DPF Suite](https://github.com/ailev/FPF/tree/main/Engineering%20DPF%20Suite) также не входит в поставку.** Включены десять избранных авторских предметных рамок:
...
Десять авторских DPF закреплены на commit `453144eda2870a14d47fd9c066deeb0405eee86f`. Авторская дата — 20 September 2026; revision хранит commit отдельно. Raw bytes не изменены. Вместе с сохранённым SDLC 0.1.0 база содержит 11 DPF и 188 адресуемых методов.
...
Собственные материалы iEWR распространяются по отдельной [лицензии](LICENSE). Семь оригинальных DPF Анатолия Левенчука лицензированы по CC BY 4.0. Точные лицензии, сведения об авторстве, редакции и контрольные суммы, отсутствие локальных правок и границы прав на сторонние материалы и программный код сохранены в [уведомлении об источниках](frameworks/dpf/NOTICE.md).
